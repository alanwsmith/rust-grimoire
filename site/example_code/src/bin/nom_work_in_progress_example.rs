...
// fn parse(p: &mut Page) -> IResult<String, String> {
fn parse(p: &mut Page) {
    let (_, strongs) = many_till(preprocess_strong, eof)(p.text.as_str()).unwrap();
    dbg!(&strongs);
    dbg!(&strongs.0.concat().concat());
    p.text = strongs.0.concat().concat();
    // p.text = strongs.0.join("").to_string();
    // let strongs_vec = strongs.0;
    // dbg!(strongs_vec);
    // p.text = strongs_vec.join("");
    // let (_, strongs3) = many_till(preprocess_strong, eof)(p.text.as_str()).unwrap();
    // Ok(("".to_string(), "".to_string()))
}

// fn preprocess_strong(source: &str) -> IResult<&str, &str> {
//     Ok(("", "asdf"))
// }
